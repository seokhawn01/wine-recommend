...
| OCR 파싱   | Google Cloud Vision API + GPT-4o-mini                    |
...
**Google Cloud Vision API 세부**

- 엔드포인트: `POST https://vision.googleapis.com/v1/images:annotate`
- 인증: `GOOGLE_VISION_API_KEY`
- 요청: Base64 인코딩 이미지
- 응답: `textAnnotations[].description` 사용
- 무료 한도: 월 1,000건
...
| 12  | Google Cloud Vision API → AI 서버 위임 구현               | 11 완료   |
| 13  | 통합 테스트 + 배포                                         | 전체 완료   |
...
| ✅ B-2 | ~~Supabase Auth 연동~~ → **Mock Auth 구현 완료** (실제 연동은 백엔드 API 완료 후) | B-1                     |
...
| ✅ B-8 | 와인 등록 페이지 (OCR 탭 + 수동 입력 탭 + 슬라이더) — **Mock OCR**                   | B-7                     |
| B-9 | Vercel 배포                                                                 | 전체 완료                   |
